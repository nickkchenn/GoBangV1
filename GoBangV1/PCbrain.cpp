...
void Brain::getChess(int chess[9], const int state[15][15], Position position, int color, int direction) {
...
int Brain::judgeType(const int chess[9]) {
	int mycolor = chess[4];
	int hiscolor;
...
int Brain::getType(const int state[15][15], Position position, int color, int direction) {
...
int Brain::judgeChessSituation(const int state[15][15], Position position, int color) {
...
int Brain::giveScore(Situation situation) {
...
Position Brain::maxScore(const int myscore[15][15], const int hisscore[15][15]) {
...
Position Brain::getPosition(const int chesspadstate[15][15], int color) {//计算下一步棋子的位置
	//旗手和棋子的标志 + 1 = 棋盘的标志
