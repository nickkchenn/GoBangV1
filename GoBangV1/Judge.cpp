...
int Brain::judgeResult(const int chesspadstate[15][15]) {
	...
	flag = 0;
	for (i = 0; i < 15; i++) {                                  //最新一步的坐标
		for (j = 0; j < 15; j++)
			if (chesspadstate[i][j]>2) {
				lastrow = i;
				lastcolumn = j;
				result = chesspadstate[i][j] - 2;                //返回当前棋手的颜色标志
				flag = 1;
				break;
			}
		if (flag)
...
/*	if (result == 1){
		...
		switch (bansign){
		case BANSIGN1:
			return result = 11;                                   //返回第一类三三禁手
			//break;
		case BANSIGN2:
			return result = 12;                                   //返回第二类三三禁手
			//break;
		case BANSIGN3:
			return result = 13;                                   //返回第三类三三禁手
			//break;
		case BANSIGN4:
			return result = 14;                                   //返回第一类四四禁手
			//break;
		case BANSIGN5:
			return result = 15;                                  //返回第二类四四禁手
			//break;
		case BANSIGN6:
			return result = 16;                                  //返回长连禁手
			//break;
		case NOBANSIGN:
			break;
		default:
